...
exports.cartAdd = function (req, res) {
  ...
  if (userId == null) {
    ...
    let get_cart =
      "SELECT ANY_VALUE(`cart_product`.quantity) as quantity, SUM(cart_product.quantity) as cartquantity, ANY_VALUE(`products`.id) as productId, ANY_VALUE(`products`.product_type_id) as productType, ANY_VALUE(`products`.name) as name, ANY_VALUE(`products`.price) as price, ANY_VALUE(`cart_product`.on_rental) as on_rental, ANY_VALUE(`products`.quantity) as productQuantity, ANY_VALUE(`products`.quantity) as actualQuantity, ANY_VALUE(`products`.image) as image, ANY_VALUE(`products`.status) as productStatus, ANY_VALUE(`products`.discount) as discount, ANY_VALUE(`products`.slug) as slug, ANY_VALUE(`cart_product`.cart_id) as cart_id, `cart_product`.product_id as product_id, ANY_VALUE(`cart_product`.quantity) as quantity,  ANY_VALUE(`cart`.cart_id) as CartId, ANY_VALUE(`cart`.user_id) as user_id, ANY_VALUE(`cart`.status) as status, ANY_VALUE(`products_images`.grocery_image) as groceyImg, ANY_VALUE(`product_variables`.unit_price) as groceyProductPrice, ANY_VALUE(`product_variables`.unit_discount) as unitDiscount FROM `products` LEFT JOIN `cart_product` ON `products`.id = `cart_product`.product_id LEFT JOIN `cart` ON `cart_product`.cart_id = `cart`.cart_id LEFT JOIN `products_images` ON `products`.id = `products_images`.product_id  LEFT JOIN `product_variables` ON `products`.id = `product_variables`.product_id where `cart`.status='0' and `cart`.user_id = '" +
      userId +
