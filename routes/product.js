...
exports.viewProduct = function(req, res, next){
    ...
    if(Ids!=""){
        ...
        let sql2 = "SELECT p.*, c.name as categoryname from products p left join books_category c on c.id=p.cat_id where p.`status`=1 and p.`product_type_id`=1 and p.id='"+Ids+"'";
        var query = db.query(sql2, function(error, result) {
            if(error) throw new Error('failed to connect category');
            ...
            var query = db.query(getsetting, function(error, settingData) {
                ...
                var sql = "SELECT * FROM `products` WHERE `status`=1 and `product_type_id`=1 order by `id` desc limit 10 ";
                var query = db.query(sql, function(error, get_products){
                    if(error) throw new Error('failed to connect products');
                    var sql_plan = "SELECT MAX(plan_price) as MaxPlanPrice FROM plans";
                    var query = db.query(sql_plan, function(error, maxplan){
                        ...
                        var userId = req.session.userId;
                        //if(userId == null){
                            let sql3 = "SELECT * FROM `products` WHERE `status`=1 and `product_type_id`=1 and id='"+Ids+"'";
                            ...
                            if(result.length > 0){
                                ...
                                res.render('front/productview', {getCartData:'',get_products:get_products,maxplan:maxplan,'viewlist': result[0],'title':title,'categorylist': category,message:req.flash('message'),csrfToken: req.csrfToken(),cryptr:cryptr,usersdata:'',data:'',settingData:settingData[0],M_title,M_description});
                                } else {
                                    res.redirect('/');
                                }
                            });
...
exports.allProducts = (req, res, next) => {
    try{
        ...
        if(typeof page == 'undefined'){
            prodsQuery = "SELECT * FROM products where status=1 and product_type_id=1 order by id desc limit "+perPage+" ";
        } else {
            var offset = parseInt((page - 1)) * parseInt(perPage); 
            prodsQuery = "SELECT * FROM products where status=1 and product_type_id=1 order by id desc limit "+perPage+" OFFSET "+offset;
        }
        ...
        var query = db.query(prodsQuery, function(error, results, fields){
            if(error) throw error;
                let total_product = "SELECT * FROM products where status=1 and product_type_id=1";
...
exports.membershipPlan = (req, res) => {
    ...
    let query = db.query(sql, function(err, result){
        ...
        var query = db.query(sql2, function(err, getplans){
            if (err) throw err;
            var sql_product_name = "SELECT `name` FROM `products` WHERE id='"+req.session.productId+"'";
            var query = db.query(sql_product_name, function(error, results){
                if(error) throw error;
                var get_rand_pro = "SELECT p.id,p.name,p.description,p.price,p.discount,p.image,p.isbn,p.status,p.author,p.book_edition,p.slug,c.name as categoryname from products p left join books_category c on c.id=p.cat_id where p.status='1' ORDER BY rand() limit 10";
                var query = db.query(get_rand_pro, function(error, get_most_view){
                    if(error) throw error;
                    let hot_pro = "SELECT `id`,`name`,`description`,`price`,`discount`,`image`,`status`,`slug` FROM `products` WHERE `status`='1' and discount >= 50 ORDER BY id desc limit 10";
                        //console.log(hot_pro);
                            ...
                            var query = db.query(sql_plan, function(error,maxplan){
                            if(results.length>0){
                                res.render('front/membership',{cryptr:cryptr,maxplan:maxplan,get_most_view:get_most_view,'getplans':getplans,'title':title,'categorylist': result,'product_name':results[0].name,csrfToken: req.csrfToken(),get_get_hot_pro:get_get_hot_pro}); 
                            } else {
                                res.render('front/membership',{cryptr:cryptr,maxplan:maxplan,get_most_view:get_most_view,'getplans':getplans,'title':title,'categorylist': result, csrfToken: req.csrfToken(), 'product_name':'',get_get_hot_pro:get_get_hot_pro});
                            }
                        });
                    });
                });
...
exports.getCategories = (req,res,next)=>{
    var qs = req.query;
    let sql = "SELECT * FROM products where is_deleted=0 and cat_id='"+qs.cat_id+"'";
    let query = db.query(sql, function(err, result){
        let sql2 = "SELECT * from books_category where is_deleted=0";
        let query = db.query(sql2, function(error, getcategory){
            var sql_plan = "SELECT MAX(plan_price) as MaxPlanPrice FROM plans";
                var query = db.query(sql_plan, function(error, maxplan){
                    res.render('front/productbycategory',{'categorylist': getcategory, 'productlist': result, maxplan:maxplan,cat_name:qs.cat_name});
                });
        });
    });
};
    ...
    try{
        ...
        for (let i = 1; i <= productId.length; i++) {
            ...
            if(productId[i-1] && get_quantity[i-1]){
                var productQnty = "SELECT name as p_name,quantity FROM `products` WHERE `id` = '"+productId[i-1]+"' and `status`='1'";
                var product_name = '';
                ...
                var query = db.query(productQnty, function(error, getgetQty){
                    ...
                    if(getQty >= get_quantity[i-1]){
                        var updatecart12 = "UPDATE `cart_product`, `cart` SET `quantity`='"+get_quantity[i-1]+"' WHERE `cart_product`.product_id='"+productId[i-1]+"' and `cart_product`.`cart_id`=`cart`.`cart_id` and `cart`.status='0' and `cart`.user_id = '"+userId+"' ";
                        ...
                        //console.log(1);
                        var msg = 'only '+getQty+ ' books available for ' +product_name+' this';
                        //console.log(msg);
                        msg_error.push(msg);
    ...
    if(req.method == "POST"){
        ...
        if(userId == null){
           ...
           /* if(!username){
            ...
            let userdata = "SELECT name,mobile,email from `users` WHERE id='"+userId+"'";
            ...
            let dataquery = db.query(userdata,function(error,getdata){
                ...
                let sql = "INSERT INTO `preorder_products` (product_id,user_id,name,email,mobile,message,created_at) VALUES('"+product_id+"','"+userId+"','"+username+"','"+useremail+"','"+usermobile+"','"+message+"','"+today+"')";
                
                let query = db.query(sql, function(error,preorder){
                    if(error) throw new Error('Pre-order insert problem');
                    let update_product = "UPDATE `products` SET pre_order_status='1' WHERE id='"+product_id+"'";
                    let querys = db.query(update_product, function(error,updateproductstbl){
                        if(error) throw new Error('pre order status not updated in products tbl');
                        req.flash('message','Your pre-order request has been sent to admin they will call back soon.');
