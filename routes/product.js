...
exports.cartAdd = function (req, res) {
  ...
  if (userId == null) {
    ...
    let get_cart =
      "SELECT cp.quantity, cp.cartquantity, `products`.id as productId, `products`.product_type_id as productType, `products`.name, `products`.price, cp.on_rental, `products`.quantity as actualQuantity, `products`.image, `products`.status, `products`.discount, `products`.slug, cp.cart_id, cp.product_id, `cart`.cart_id as CartId, `cart`.user_id, `cart`.status, `products_images`.grocery_image as groceyImg, `product_variables`.unit_price as groceyProductPrice, `product_variables`.unit_discount as unitDiscount FROM (SELECT `cart_product`.cart_id, `cart_product`.product_id, SUM(`cart_product`.quantity) as quantity, SUM(`cart_product`.quantity) as cartquantity, MAX(`cart_product`.on_rental) as on_rental FROM `cart_product` LEFT JOIN `cart` ON `cart_product`.cart_id = `cart`.cart_id WHERE `cart`.status='0' and `cart`.user_id = '" +
      userId +
      "' GROUP BY `cart_product`.cart_id, `cart_product`.product_id) cp LEFT JOIN `products` ON `products`.id = cp.product_id LEFT JOIN `cart` ON cp.cart_id = `cart`.cart_id LEFT JOIN `products_images` ON `products`.id = `products_images`.product_id AND `products_images`.id = (SELECT MIN(pi.id) FROM `products_images` pi WHERE pi.product_id = `products`.id) LEFT JOIN `product_variables` ON `products`.id = `product_variables`.product_id AND `product_variables`.id = (SELECT MIN(pv.id) FROM `product_variables` pv WHERE pv.product_id = `products`.id)";
