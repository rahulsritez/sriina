exports.stateDeliveryCharge = function(req, res){
    ...
    let query = db.query(sql, function(err, result){
        res.render('admin/statelist',{'title':title,'productlist': result,message:req.flash('message')});
    });
}
...
exports.updateStateDeliveryCharge = (req, res)=>{
	if(req.url == '/updatestatedeliverycharge'){
		let data = req.body;
		let slqupdate = "UPDATE `state1` set `delivery_charges`='"+data.edit_shipping_charge+"', `status`='"+data.edit_status+"' where `id`='"+data.edit_id+"'";
