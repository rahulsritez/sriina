exports.uploadExcel = (req, res) => {
    if(req.method == "GET"){
        ...
        if(userId == null && userType==null){
            ...
            res.render('admin/uploadexcel',{message:req.flash('message'), errors:req.flash('errors'), csrfToken: req.csrfToken()});
        }
    }
}
exports.uploadExcelFile = (req, res, next) => {
    if(req.url == '/uploadexcelfile'){
        ...
        readXlsxFile(exFile).then((rows) => {
            ...
            let query = 'INSERT INTO products (isbn13, name, author, publisher, book_edition, book_language, book_binding, currency_code, price, weight, delivery_charge, quantity, discount, publishing_year, description, no_of_pages, image, cat_id, product_type_id, created_at) VALUES ?';
    ...
    form.parse(req, function (err, fields, files) {
        ...
        readXlsxFile(exFile).then(async (rows) => {
            ...
            const promises = newBooksArr.map(async (row, index, array) => {
                ...
                var slug_url = slugify(data['name'],{replacement: '-',remove: /[,*+~.(){}'"\/\\#$%<>?!:@]/g, lower: true,strict: false});
                data['slug'] = slug_url;
                data['user_id'] = userId;
                //data['cat_id'] = 0;
                ...
                if(currCode && price) {
                    const finalAmount = await currencyConverter.from(currCode).to("INR").amount(price).convert().then((response) => {
                        return response;
                    })
                    if(finalAmount) {
                ...
                if(data['isbn13'] && (data['isbn13'].length > 13 || data['isbn13'].length < 13)){
                    is_valid_record = false;
                    failed_by_isbn13 += 1;
                }
                ...
                if(is_valid_record) {
                    var sql2 = "SELECT * FROM `products` WHERE  status=1 and (`isbn` ='"+data['isbn']+"' or `isbn13` ='"+data['isbn13']+"')";
                    let results = await new Promise((resolveExist, reject) => db.query(sql2, (err, results) => {
                        if (err) {
                            reject(err)
                        } else {
                ...
                if(is_valid_record) {
                    ...
                    let sql1 = "INSERT INTO products("+fields+") VALUES ("+fieldsValues+")";
                    let results = await new Promise((resolveNew, reject) => db.query(sql1, (err, results) => {
                        if (err) {
                            reject(err)
                        } else {
                            resolveNew(results);
            ...
            await Promise.all(promises).then(()=>{
                ...
                if(failed == total){
                    req.flash('message',`${uploaded} records uploaded out of ${total} records.`);
                    failed_by_name ? req.flash('errors', `${failed_by_name} failed due to name not valid`) : '';
                    failed_by_isbn ? req.flash('errors', `${failed_by_isbn} failed due to isbn not found`) : '';
                    failed_by_isbn10 ? req.flash('errors', `${failed_by_isbn10} failed due to isbn only 10 digits`) : '';
                    failed_by_isbn13 ? req.flash('errors', `${failed_by_isbn13} failed due to isbn13 only 13 digits`) : '';
                    ...
                    failed_by_price ? req.flash('errors', `${failed_by_price} failed due to price value not found`) : '';
                    failed_by_exist ? req.flash('errors', `${failed_by_exist} failed due to record already exists`) : '';
                    errors ? req.flash('error', `${errors} failed to getting error`) : '';
                } else {
                    req.flash('message',`Bulk contacts successfully added. Total ${uploaded} uploaded, ${failed} failed out of ${total} records.`);
                    if(failed){
                        failed_by_name ? req.flash('errors', `${failed_by_name} failed due to name not valid`) : '';
                        failed_by_isbn ? req.flash('errors', `${failed_by_isbn} failed due to isbn not found`) : '';
                        failed_by_isbn10 ? req.flash('errors', `${failed_by_isbn10} failed due to isbn only 10 digits`) : '';
