...
exports.subcategorypage = function (req, res) {
  ...
  var query = db.query(sql1, function (err, result) {
    var sql2 =
      "select s.* ,c.name as categoryname from books_sub_category s left join books_category c on c.id=s.categoryid where s.is_deleted=0";
    var query = db.query(sql2, function (err, result1) {
...
exports.Deletesubcategory = function (req, res) {
  ...
  var data = "";
  var sql1 =
    "UPDATE books_sub_category set is_deleted=1  where id='" + id + "'";
  var query = db.query(sql1, function (err, result) {
    if (err) {
      console.log(err);
    } else {
