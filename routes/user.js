...
exports.statepage = function (req, res) {
  ...
  if (userId == null) {
    ...
    var query = db.query(sql1, function (err, result) {
      var sql2 =
        "Select s.* ,c.name as countryname from state s left join country c on c.id=s.countryid where s.is_deleted=0";
      var query = db.query(sql2, function (err, result1) {
...
exports.catalogcondition = function (req, res) {
  ...
  let sql1 =
    "Select * from catalog_condition where is_deleted=0 order by id desc";
  let query = db.query(sql1, function (error, result) {
    if (error) throw error;
    let sql2 = db.query(
      "Select DISTINCT publisher from products where is_deleted=0 order by id desc",
      function (err, ress) {
        let sql3 = db.query(
          "Select DISTINCT author from products where is_deleted=0 order by id desc",
          function (err, resss) {
            let sql4 = db.query(
              "Select DISTINCT url_name from category_url where status=1 order by id desc",
              function (err, cateress) {
                let sql5 = db.query(
                  "Select DISTINCT name from catalog_info where is_deleted=0 order by id desc",
                  function (err, catalogress) {
...
exports.subcategorypage = function (req, res) {
  ...
  var query = db.query(sql1, function (err, result) {
    var sql2 =
      "select s.* ,c.name as categoryname from subcategory s left join category c on c.id=s.categoryid where s.is_deleted=0";
    var query = db.query(sql2, function (err, result1) {
...
exports.Editstate = function (req, res) {
  ...
  var data = "";
  var sql1 =
    "Select s.* ,c.name as countryname from state s left join country c on c.id=s.countryid where s.id='" +
    id +
    "'";
...
exports.Editcity = function (req, res) {
  ...
  var query = db.query(sql1, function (err, result) {
    if (err) {
      ...
      var sql2 =
        "select id , name from state where countryid='" +
        result[0]["countryid"] +
        "'";
      var query = db.query(sql2, function (err, result1) {
        //console.log(result1);
...
exports.Deletecatalogcondition = function (req, res) {
  ...
  var data = "";
  var sql1 =
    "UPDATE catalog_condition set is_deleted='1' where id='" + id + "'";
  var query = db.query(sql1, function (err, result) {
    if (err) {
      console.log(err);
    } else {
...
exports.Addsubcategory = function (req, res) {
  ...
  if (req.url == "/addsubcategory") {
    ...
    form.parse(req, function (err, fields, files) {
      ...
      var sql1 =
        "INSERT INTO `subcategory`(name,image,categoryid,status,created_on,modeified_on) VALUES ('" +
        subcategoryname +
        "', '" +
        subcategoryimage +
        "', '" +
...
exports.Editsubcategory = function (req, res) {
  ...
  var data = "";
  var sql1 =
    "select s.* ,c.name as categoryname,c.id as categoryid from subcategory s left join category c on c.id=s.categoryid where s.id='" +
    id +
    "'";
  var query = db.query(sql1, function (err, result) {
    if (err) {
...
exports.Updatesubcategory = function (req, res) {
  ...
  if (req.url == "/updatesubcategory") {
    ...
    form.parse(req, function (err, fields, files) {
      ...
      if (subcategoryimage) {
        var newpath =
          "./public/admin/images/" + files.editsubCategoryimage.name;
        fs.readFile(oldpath, function (err, data) {
          if (err) throw err;
          console.log("File read!");
...
exports.Editproduct = function (req, res) {
  ...
  var query = db.query(sql1, function (err, result) {
    if (err) {
      ...
      var sql2 =
        "select id , name from subcategory where categoryid='" +
        result[0]["categoryid"] +
        "'";
      var query = db.query(sql2, function (err, result1) {
        console.log(result1);
...
exports.AdminUpdateProduct = (req, res, next) => {
  ...
  if (req.url == "/adminupdateproduct") {
    ...
    form.parse(req, function (err, fields, files) {
      ...
      var edit_publishing_year = fields.edit_publishing_year.replace(
        /'/g,
        "''"
      );
      ...
      var edit_delivery_charge = fields.edit_delivery_charge
        ? fields.edit_delivery_charge
        : 0;
      ...
      var edit_meta_description = fields.edit_meta_description.replace(
        /'/g,
        "''"
      );
...
exports.addPages = (req, res) => {
  ...
  if (req.method == "POST") {
    ...
    var sql =
      "INSERT INTO pages(title,description,status,created_at) VALUES('" +
      title +
      "','" +
      description +
      "','0','" +
    ...
    var query = db.query(sql, function (error, results) {
      if (results.length > 0) {
        ...
        var query = db.query(SQL, function (error, updatePwd) {
          ...
          req.flash(
            "message",
            "Password has been updated on this email Id :" + email
          );
          res.redirect("customerpage");
        });
      } else {
...
exports.menuPagesPost = (req, res, next) => {
  if (req.method == "POST") {
    ...
    var sql =
      "INSERT INTO menu(name,status,created_at,updated_at) VALUES('" +
      name +
      "','1','" +
      today +
      "','" +
...
exports.addStateonRent = (req, res) => {
  if (req.method == "POST") {
    ...
    let checkcode =
      "SELECT * FROM state_on_rent WHERE name='" +
      name +
      "' and code='" +
      code +
      "'";
...
exports.updatePermission = (req, res) => {
  if (req.method == "POST") {
    ...
    let edit_id = req.body.edit_id;
    let sql =
      "UPDATE settings SET permission='" +
      permission +
      "', created_at='" +
      today +
      "' WHERE id='" +
...
exports.getGrocerySubCategory = function (req, res) {
  var id = req.body.id;
  var sql =
    "SELECT id,name FROM grocery_sub_category WHERE grocery_id='" +
    id +
    "' and status=1";
  var query = db.query(sql, function (error, results) {
