...
exports.login = function (req, res) {
    ...
    if (req.method == "POST") {
        ...
        var sql = "SELECT `id`, `name`, `type`, `password` FROM `users` WHERE `email`='" + email + "' and status='0' and (`type`='1' || `type`='2' || `type`='3' || `type`='5') ";
    ...
    if (req.method == "POST") {
        ...
        var query = db.query(sql, function (err, result) {
            ...
            if (result.length > 0) {
                ...
                var sql1 = "INSERT INTO `users`(`name`,`email`, `password`,createdby,modifiedby) VALUES ('" + name + "', '" + email + "', '" + hash + "','" + today + "','" + today + "')";
    ...
    var query = db.query(sql1, function (err, result) {
        if (userId == null) {
            ...
            if (req.session.type == 1) {
                if (req.query.search) {
                    var sql2 = "SELECT p.*,c.name as categoryname from products p left join books_category c on c.id=p.cat_id WHERE p.status=1 and (p.name LIKE '%" + req.query.search + "%' ) and product_type_id=1 ORDER BY id DESC LIMIT " + limit_start + ' OFFSET ' + limit_end;
                    countQuery = "SELECT count(*) as count from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1  and (p.name LIKE '%" + req.query.search + "%' ) order by c.id desc  "
                } else {
                    var sql2 = "SELECT p.*,c.name as categoryname from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1 order by id desc LIMIT " + limit_start + ' OFFSET ' + limit_end;
                    countQuery = "SELECT count(*) as count from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1 order by c.id desc  "

                }

            } else if (req.session.type == 2) {
                var sql2 = "SELECT p.*,c.name as categoryname from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1 and `user_id`!='0' and p.user_id='" + userId + "'  order by p.id desc LIMIT " + limit_start + ' OFFSET ' + limit_end;
                countQuery = "SELECT count(*) as coun from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1 and `user_id`!='0' and p.user_id='" + userId + "'  order by p.id desc ";
            } else if (req.session.type == 5) {
                var sql2 = "SELECT p.*,c.name as categoryname from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1 and `user_id`!='0' order by id desc LIMIT " + limit_start + ' OFFSET ' + limit_end;
                countQuery = "SELECT count(*) as count from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1 and `user_id`!='0' order by id desc ";
            } else {
                var sql2 = "SELECT p.*,c.name as categoryname from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1 and `user_id`='" + userId + "' and `user_id`!='0' order by p.id desc LIMIT " + limit_start + ' OFFSET ' + limit_end;
                countQuery = "SELECT count(*) as count from products p left join books_category c on c.id=p.cat_id where p.status=1 and product_type_id=1 and `user_id`='" + userId + "' and `user_id`!='0' order by p.id desc ";
            }
            ...
            db.query(sql2, function (err, result1) {
                db.query(countQuery, function (err, total) {
                    ...
                    const totalpages = total?.[0]?.count / req.query.count > Math.ceil(total?.[0]?.count / req.query.count) ? Math.ceil(total?.[0]?.count / req.query.count) + 1 : Math.ceil(total?.[0]?.count / req.query.count);
                    db.query(get_type, function (err, get_type_data) {
                        res.render('admin/product', { 'categorylist': result, "total": total?.[0].count, 'productlist': result1, get_type_data: get_type_data, message: req.flash('message'), errors: req.flash('errors'), query: req.query, totalpages: totalpages })
                    });
                });
            })
        }
...
exports.citypage = function (req, res) {
    ...
    var sql1 = "Select * from country where is_deleted=0";
    var query = db.query(sql1, function (err, result) {
        var sql2 = 'Select cs.* ,c.name as countryname ,s.name as statename from city cs left join country c on c.id=cs.countryid left join state s on s.id=cs.stateid where cs.is_deleted=0';
        var query = db.query(sql2, function (err, result1) {
            res.render('city.ejs', { 'countrylist': result, 'citylist': result1, 'statelist': rsult2 })
        });
    });
};
...
exports.categorypage = function (req, res) {
    ...
    let query = db.query(sql1, function (error, result) {
        ...
        res.render('admin/category', { 'categorylist': result, message: req.flash('message') })
    });
};
//catalog condition
            ...
            let sql3 = db.query("Select DISTINCT author from products where is_deleted=0 order by id desc", function (err, resss) {
                let sql4 = db.query("Select DISTINCT url_name from category_url where status=1 order by id desc", function (err, cateress) {
                    let sql5 = db.query("Select DISTINCT name from catalog_info where is_deleted=0 order by id desc", function (err, catalogress) {
                        ...
                        res.render('admin/catalog-condition', { 'conditionlist': result, 'publisher': ress, 'category': cateress, 'author': resss, 'catalog': catalogress, message: req.flash('message') })
                    });
                });
            });
        });
    ...
    let query = db.query(sql1, function (error, result) {
        ...
        let sql2 = db.query("Select DISTINCT publisher from products where is_deleted=0 order by id desc", function (err, ress) {
            let sql3 = db.query("Select DISTINCT author from products where is_deleted=0 order by id desc", function (err, resss) {
                ...
                res.render('admin/catalog', { 'cataloglist': result, 'publisher': ress, 'author': resss, message: req.flash('message') })
            });
        });
    });
};
...
exports.cartspage = function (req, res) {
    ...
    let query = db.query(sql1, function (error, result) {
        if (error) throw error;
        res.render('admin/carts', { 'cataloglist': result, message: req.flash('message') })
    });
};
...
exports.subcategorypage = function (req, res) {
    ...
    var query = db.query(sql1, function (err, result) {
        var sql2 = "select s.* ,c.name as categoryname from subcategory s left join category c on c.id=s.categoryid where s.is_deleted=0";
        var query = db.query(sql2, function (err, result1) {
            res.render('subcategory.ejs', { 'categorylist': result, 'subcategorylist': result1 })
        });
    });
...
exports.customerpage = (req, res, next) => {
    ...
    var query = db.query(sql1, function (error, result) {
        ...
        res.render('admin/customer', { 'customerlist': result, 'title': title, csrfToken: req.csrfToken(), message: req.flash('message') });
    });
};
...
exports.Addcountry = function (req, res) {
    if (req.method == "POST") {
        ...
        var today = new Date().toISOString().slice(0, 19).replace('T', ' ');
        var sql1 = "INSERT INTO `country`(name,code, isd_code,status,created_on,modified_on,currency) VALUES ('" + Country + "', '" + CountryCode + "', '" + ISDCode + "','" + Status + "','" + today + "' ,'" + today + "' ,'" + Currency + "')";
        var query = db.query(sql1, function (err, result) {
            if (err) {
                console.log(err);
            } else {
...
exports.Updatecountry = function (req, res) {
    if (req.method == "POST") {
        ...
        var sql1 = "UPDATE country set name='" + Country + "' , code='" + CountryCode + "',isd_code='" + ISDCode + "',status='" + Status + "',currency='" + Currency + "',created_on='" + today + "',modified_on='" + today + "' where id='" + id + "'";
        var query = db.query(sql1, function (err, result) {
            if (err) {
                console.log(err);
            } else {
...
exports.Addstate = function (req, res) {
    if (req.method == "POST") {
        ...
        var sql1 = "INSERT INTO `state`(name,code, countryid,status,created_on,modeified_on) VALUES ('" + State + "', '" + stateCode + "', '" + countryid + "','" + Status + "','" + today + "' ,'" + today + "' )";
        var query = db.query(sql1, function (err, result) {
            if (err) {
                console.log(err);
            } else {
...
exports.Updatestate = function (req, res) {
    if (req.method == "POST") {
        ...
        var sql1 = "UPDATE state set name='" + State + "' , code='" + StateCode + "',countryid	='" + Countryid + "',status='" + Status + "',created_on='" + today + "',modeified_on='" + today + "' where id='" + id + "'";
        var query = db.query(sql1, function (err, result) {
            if (err) {
                console.log(err);
            } else {
...
exports.Addcity = function (req, res) {
    if (req.method == "POST") {
        ...
        var sql1 = "INSERT INTO `city`(name,code, countryid,stateid,status,created_on,modified_on) VALUES ('" + city + "', '" + code + "', '" + countryid + "' , '" + stateid + "' ,'" + Status + "','" + today + "' ,'" + today + "' )";
        var query = db.query(sql1, function (err, result) {
            if (err) {
                console.log(err);
            } else {
...
exports.Updatecity = function (req, res) {
    if (req.method == "POST") {
        ...
        var sql1 = "UPDATE city set name='" + City + "' , code='" + cityCode + "',countryid='" + countryid + "',stateid='" + stateid + "',status='" + Status + "',created_on='" + today + "',modified_on='" + today + "' where id='" + id + "'";
        var query = db.query(sql1, function (err, result) {
            if (err) {
                console.log(err);
            } else {
    ...
    if (req.method == "POST") {
        ...
        var query = db.query(sql, function (error, checkusers) {
            if (checkusers.length > 0) {
                ...
                var sql1 = "INSERT INTO `users`(name, email, mobile, password, type, status,createdby,modifiedby) VALUES ('" + name + "', '" + email + "', '" + mobile + "', '" + hash + "', '0', '0', '" + today + "', '" + today + "')";
                var query = db.query(sql1, function (error, result) {
    ...
    if (req.method == "POST") {
        ...
        var query = db.query(sql, function (error, checkusers) {
            if (checkusers.length > 1) {
                ...
                var sql1 = "UPDATE users set `name`='" + name + "', `email`='" + email + "', `mobile`='" + mobile + "', `status`='" + Status + "', `modifiedby` ='" + today + "' where `id`='" + id + "'";
                var query = db.query(sql1, function (error, result) {
    ...
    if (req.url == '/addcategory') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            if (category_image) {
                ...
                var sql1 = "INSERT INTO `books_category`(name,image,status,created_at ,updated_at) VALUES ('" + categoryname + "', '" + category_image + "', '" + status + "','" + today + "','" + today + "')";
                var query = db.query(sql1, function (error, result) {
                    if (error) throw new Error('Category Image Problem');
                    res.redirect("/categorypage");
                });
            } else {
                var sql1 = "INSERT INTO `books_category`(name,status,created_at,updated_at) VALUES ('" + categoryname + "','" + status + "','" + today + "','" + today + "')";
                var query = db.query(sql1, function (error, result) {
    ...
    if (req.url == '/updatecategory') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            if (categoryimage) {
                ...
                var sql1 = "UPDATE books_category set name='" + categoryname + "', image='" + categoryimage + "', status='" + status + "', meta_title ='" + edit_meta_title + "', meta_description ='" + edit_meta_description + "', meta_canonical_tag ='" + edit_meta_canonical_tag + "', category_text ='" + edit_category_text + "', updated_at='" + today + "' where id='" + edit_id + "'";
                var query = db.query(sql1, function (error, result) {
                    if (error) throw error;
                ...
                var sql1 = "UPDATE books_category set name='" + categoryname + "' ,status='" + status + "', meta_title ='" + edit_meta_title + "', meta_description ='" + edit_meta_description + "', meta_canonical_tag ='" + edit_meta_canonical_tag + "', category_text ='" + edit_category_text + "', updated_at='" + today + "' where id='" + edit_id + "'";
                var query = db.query(sql1, function (error, result) {
                    if (error) throw error;
                    req.flash('message', 'Category has been successfully updated.');
                    res.redirect("/categorypage");
...
exports.Updatecartts = function (req, res) {
    ...
    if (req.url == '/updatecartts') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            var sql1 = "UPDATE carts set name='" + name + "' ,status='" + status + "',descs ='" + desc + "', code ='" + code + "', 	user_limit ='" + user_limit + "', valid_from='" + valid_from + "', valid_to='" + valid_to + "', minimum_amt	='" + mini_amt + "' where id='" + edit_id + "'";
            var query = db.query(sql1, function (error, result) {
                if (error) throw error;
...
exports.Updatecatalog = function (req, res) {
    ...
    if (req.url == '/updatecatalog') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            var sql1 = "UPDATE catalog_info set name='" + name + "' ,publisher ='" + pub_name + "', author ='" + auth_name + "', 	ISBN13 ='" + edit_isbn13 + "', ISBN10='" + edit_isbn10 + "',publishing_year='" + edit_publishing_year + "', discount	='" + edit_discount + "', country	='" + country + "', currency='" + currency + "', from_d='" + edit_valid_from + "', to_d='" + edit_valid_to + "', customer_limit='" + limit + "' where id='" + edit_id + "'";
            var query = db.query(sql1, function (error, result) {
                if (error) throw error;
...
exports.Addcatalogcondition = function (req, res) {
    ...
    if (req.url == '/addcatalogcondition') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            var sql1 = "INSERT INTO `catalog_condition`(category,brand,supplies,attribute,catalog_id,is_deleted) VALUES ('" + category + "','" + supplies + "','" + brand + "','" + catalog + "','" + attribute + "','0')";
        ...
        form.parse(req, function (err, fields, files) {
            ...
            if (coun2 != '10') {
                ...
                if (coun != '13') {
                    ...
                    var sql1 = "INSERT INTO `catalog_info`(name,publisher,author,ISBN13,ISBN10,publishing_year,book_binding,currency,country,catalog_group,discount,from_d,to_d,customer_limit,is_deleted) VALUES ('" + name + "','" + pub_name + "','" + auth_name + "','" + isbn13 + "','" + isbn10 + "','" + publishing_year + "','" + book_binding + "','" + currency + "','" + country + "','" + group + "','" + discount + "','" + from + "','" + to + "','" + limit + "','0')";
...
exports.Addcart = function (req, res) {
    ...
    if (req.url == '/addcarts') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            var sql1 = "INSERT INTO `carts`(name,descs,code,user_limit,valid_from,valid_to,minimum_amt,is_deleted,status) VALUES ('" + name + "','" + desc + "','" + code + "','" + userlimit + "','" + vdate + "','" + tdate + "','" + mamount + "','0','0')";
            // console.log(fields)
            var query = db.query(sql1, function (error, result) {
...
exports.Addsubcategory = function (req, res) {
    ...
    if (req.url == '/addsubcategory') {
        var post = req.body;
        //        var categoryname= post.Category_Name;
        //        var categoryimage= post.Categoryimage;
        //        console.log(categoryimage);
        //        var Status= post.Category_Status;
        var today = new Date().toISOString().slice(0, 19).replace('T', ' ');
        var form = new formidable.IncomingForm();
        form.parse(req, function (err, fields, files) {
            var oldpath = files.subCategoryimage.path;
            var status = fields.subCategory_Status;
            var subcategoryname = fields.SubCategory_Name;
            var subcategoryimage = files.subCategoryimage.name;
            var categoryid = fields.Category_Nameid;
            var newpath = './public/admin/images/' + files.subCategoryimage.name;
            fs.readFile(oldpath, function (err, data) {
                if (err) throw err;
                console.log('File read!');
            ...
            var sql1 = "INSERT INTO `subcategory`(name,image,categoryid,status,created_on,modeified_on) VALUES ('" + subcategoryname + "', '" + subcategoryimage + "', '" + categoryid + "', '" + status + "' ,'" + today + "' ,'" + today + "' )";
            var query = db.query(sql1, function (err, result) {
                if (err) {
                    console.log(err);
                } else {
...
exports.Updatesubcategory = function (req, res) {
    ...
    if (req.url == '/updatesubcategory') {
        ...
        var form = new formidable.IncomingForm();
        form.parse(req, function (err, fields, files) {
            var oldpath = files.editsubCategoryimage.path;
            var id = fields.subcategory_id;
            var status = fields.editsubCategory_Status;
            var subcategoryname = fields.editSubCategory_Name;
            var categoryid = fields.editCategory_Nameid;
            var subcategoryimage = files.editsubCategoryimage.name;
            if (subcategoryimage) {
                var newpath = './public/admin/images/' + files.editsubCategoryimage.name;
                fs.readFile(oldpath, function (err, data) {
                    if (err) throw err;
                    console.log('File read!');
                ...
                var sql1 = "UPDATE subcategory set name='" + subcategoryname + "' , categoryid='" + categoryid + "' , image='" + subcategoryimage + "',status='" + status + "',created_on='" + today + "',modeified_on='" + today + "' where id='" + id + "'";
                var query = db.query(sql1, function (err, result) {
                    if (err) {
                        console.log(err);
                    } else {
                ...
                var sql1 = "UPDATE subcategory set name='" + subcategoryname + "' , categoryid='" + categoryid + "' ,status='" + status + "',created_on='" + today + "',modeified_on='" + today + "' where id='" + id + "'";
                var query = db.query(sql1, function (err, result) {
                    if (err) {
                        console.log(err);
                    } else {
...
exports.Deletesubcategory = function (req, res) {
    var id = req.body.id
    var data = '';
    var sql1 = "UPDATE subcategory set is_deleted=1  where id='" + id + "'";
    var query = db.query(sql1, function (err, result) {
        if (err) {
            console.log(err);
        } else {
...
exports.Addproduct = function (req, res) {
    ...
    if (req.url == '/addproduct') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            var slug_url = slugify(product_name, { replacement: '-', remove: /[,*+~.(){}'"\/\\#$%<>?!:@]/g, lower: true, strict: false });
            ...
            let sql1 = "INSERT INTO products(`user_id`,`product_type_id`,`name`,`author`,`publisher`,`book_edition`,`price`,`discount`,`delivery_charge`,`quantity`,`isbn`,`isbn13`,`language`,`book_binding`,`cat_id`,`status`,`no_of_pages`,`weight`,`image`,`description`,`slug`,`product_type`,`publishing_year`, `author_details`, `meta_title`, `meta_description`, `created_at`,`updated_at`) VALUES ('" + userId + "','" + product_type_id + "','" + product_name + "', '" + author + "','" + publisher + "','" + book_edition + "', '" + price + "', '" + discount + "','" + delivery_charge + "','" + quantity + "', '" + ISBN + "', '" + ISBN13 + "', '" + language + "', '" + book_binding + "', '" + category + "', '" + status + "', '" + no_of_pages + "', '" + weight + "', '" + productimage + "','" + descriptions + "', '" + slug_url + "', '" + product_type + "','" + publishing_year + "', '" + author_details + "', '" + meta_title + "','" + meta_description + "', '" + today + "','" + today + "')";
    ...
    if (req.url == '/updateproduct') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            if(productimage){
                ...
                var sql1 = "UPDATE product set name='" + productname + "', price='" + price + "', discount='" + discount + "', description='" + description + "', image='" + productimage + "', categoryid='" + categoryid + "', subcategoryid='" + subcategoryid + "',status='" + status + "',created_on='" + today + "',modified_on='" + today + "' where id='" + id + "'";
                var query = db.query(sql1, function (err, result) {
                    if (err) {
                        console.log(err);
                    } else {
                ...
                var sql1 = "UPDATE product set name='" + productname + "', price='" + price + "', discount='" + discount + "', description='" + description + "', categoryid='" + categoryid + "', subcategoryid='" + subcategoryid + "',status='" + status + "',created_on='" + today + "',modified_on='" + today + "' where id='" + id + "'";
                var query = db.query(sql1, function (err, result) {
                    if (err) {
                        console.log(err);
                    } else {
    ...
    if (req.url == '/adminupdateproduct') {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            if (productimage) {
                ...
                let sql1 = "UPDATE `products` SET `product_type_id` ='" + edit_product_type_id + "', `name` ='" + edit_product_name + "', `author` = '" + edit_author + "', `publisher` ='" + edit_publisher + "', `book_edition`='" + edit_book_edition + "', `price`='" + edit_price + "',`discount`='" + edit_discount + "', `delivery_charge`='" + edit_delivery_charge + "', `quantity`='" + edit_quantity + "', `isbn`='" + edit_ISBN + "', `isbn13`='" + edit_ISBN13 + "', `language`='" + edit_language + "', `book_binding`='" + edit_book_binding + "', `cat_id`='" + edit_category + "', `status`='" + edit_status + "', `no_of_pages`='" + edit_no_of_pages + "', `weight`='" + weight + "', `image`='" + productimage + "',  `product_type`='" + edit_product_type + "',  `publishing_year`='" + edit_publishing_year + "', `description`='" + edit_descriptions + "', `author_details` ='" + edit_author_details + "', `meta_title` ='" + edit_meta_title + "', `meta_description` ='" + edit_meta_description + "', `updated_at`='" + today + "' WHERE id='" + edit_id + "'";
                ...
                let sql1 = "UPDATE `products` SET `product_type_id` ='" + edit_product_type_id + "', `name` ='" + edit_product_name + "', `author` = '" + edit_author + "', `publisher` ='" + edit_publisher + "', `book_edition`='" + edit_book_edition + "', `price`='" + edit_price + "',`discount`='" + edit_discount + "', `delivery_charge`='" + edit_delivery_charge + "', `quantity`='" + edit_quantity + "', `isbn`='" + edit_ISBN + "', `isbn13`='" + edit_ISBN13 + "', `tax_included`='" + edit_included + "', `language`='" + edit_language + "',`book_binding`='" + edit_book_binding + "', `cat_id`='" + edit_category + "', `status`='" + edit_status + "', `no_of_pages`='" + edit_no_of_pages + "', `weight`='" + weight + "', `product_type`='" + edit_product_type + "',  `publishing_year`='" + edit_publishing_year + "', `description`='" + edit_descriptions + "', `author_details` ='" + edit_author_details + "', `meta_title` ='" + edit_meta_title + "', `meta_description` ='" + edit_meta_description + "', `updated_at`='" + today + "' WHERE id='" + edit_id + "'";
                var query = db.query(sql1, function (error, update) {
                    if (error) throw error;
                    req.flash('message', 'Product has been successfully updated.');
                    res.redirect("/productlist");
...
exports.webPages = (req, res, next) => {
    ...
    var query = db.query(sql, function (error, gerPages) {
        if (error) throw error;
        res.render('admin/webpage', { title: 'Pages', pageslist: gerPages, message: req.flash('message') });
    });
}
...
exports.editPages = (req, res, next) => {
    if (req.method == "POST") {
        ...
        var sql = "UPDATE pages SET title = '" + title + "', status = '" + status + "', description = '" + description + "', updated_at = '" + today + "' WHERE id = '" + edit_id + "'";
...
exports.menuPages = (req, res, next) => {
    ...
    var query = db.query(sql, function (error, menuslist) {
        if (error) throw error;
        res.render('admin/menupages', { title: 'Menu', menuslist: menuslist, csrfToken: req.csrfToken(), message: req.flash('message') });
    });
}
...
exports.Updateddiscount = (req, res, next) => {
    if (req.method == "GET") {
        ...
        var sql = "SELECT count(*) as total_products, id,discount,quantity,publisher,status,publishing_role_id,imprint_name FROM `products` WHERE `status`='1' group by imprint_name";
        //console.log(sql); return;
        var query = db.query(sql, function (error, updated_discount) {
            if (error) throw error;
            res.render('admin/updatediscount', { updated_discount: updated_discount, message: req.flash('message'), title: title });
        });
    }
}
exports.saveDiscount = (req, res, next) => {
    if (req.method == "POST") {
        ...
        var sql = "UPDATE `products` SET `discount` = '" + edit_discount + "', `updated_at` = '" + today + "' WHERE `publishing_role_id` = '" + publishing_role_id + "' and `publisher`='" + publisher + "' ";
        var query = db.query(sql, function (error, updatepages) {
            if (error) throw error;
...
exports.stateONRent = (req, res, next) => {
    ...
    let sql = db.query(data, function (error, stateonrent) {
        if (error) throw new Error('DATA PROBLEM');
        res.render('admin/stateonrent', { stateonrent: stateonrent, title: title, message: req.flash('message'), csrfToken: req.csrfToken() });
    });
};
    ...
    if (req.method == "POST") {
        ...
        let alredydata = db.query(checkcode, function (error, sqldata) {
            if (sqldata.length > 0) {
                ...
                let sql = "INSERT INTO state_on_rent(name,code,countryid,status,created_on) VALUES('" + name + "','" + code + "','30','" + status + "','" + today + "')";
...
exports.updateStateonRent = (req, res) => {
    if (req.method == "POST") {
        ...
        let sql = "UPDATE state_on_rent SET name ='" + name + "', code='" + code + "', status = '" + status + "', modeified_on='" + today + "' WHERE id='" + edit_id + "'";
...
exports.vendorPage = (req, res) => {
    var sql1 = "SELECT u.id as userId,u.name,u.email,u.mobile,u.type,u.status,vi.* from users u left join vendor_info vi on u.id=vi.user_id where u.type='3' order by u.id DESC";
    let title = 'Vendor List';
    var query = db.query(sql1, function (error, result) {
        if (error) throw new Error('Table data problem');
        res.render('admin/vendor', { 'vendorlist': result, 'title': title, csrfToken: req.csrfToken(), message: req.flash('message'), errors: req.flash('errors') });
    });
}
...
exports.updateVendor = (req, res) => {
    if (req.method == "POST") {
        ...
        if (!emailReg.test(edit_company_mail)) {
            ...
            let sql = "UPDATE `vendor_info` SET `company_mail`='" + edit_company_mail + "', `company_name`='" + edit_company_name + "', `company_phone` ='" + edit_company_phone + "', `company_contact_person` ='" + edit_company_contact_person + "',`company_contact_phone_no`='" + edit_company_contact_phone_no + "', `company_gst` ='" + edit_company_gst + "' WHERE user_id='" + edit_id + "'";

            let query = db.query(sql, function (error, updatevendor) {
                if (error) throw new Error('vendor_info tbl update problem');

                let sql_user = "UPDATE `users` SET `name`='" + edit_company_name + "', `email`='" + edit_company_mail + "', `mobile` ='" + edit_company_phone + "', `status`='" + edit_status + "' WHERE id='" + edit_id + "' and `type`='3' ";
...
exports.webSetting = (req, res) => {
    ...
    var query = db.query(sql1, function (error, result) {
        if (error) throw new Error('Table data problem');
        res.render('admin/websettings', { permissionlist: result, 'title': title, csrfToken: req.csrfToken(), message: req.flash('message'), errors: req.flash('errors') });
    });
}
