...
exports.orderpage = function(req, res){
    ...
    var query = db.query(sql1, function(err, result) {
        ...
        db.query(SQL,function(error, getproductsstatus){
            res.render('admin/order',{orderlist: result,moment:moment,csrfToken:req.csrfToken(),message:req.flash('message'), errors:req.flash('errors'),getproductsstatus:getproductsstatus })
        });
        });
};
...
exports.orderViewPage = (req, res, next) => {
    ...
    let sql_userId = "SELECT `cart_id`, `customer_id`,`paid_amount`,`created_at`,`bk_order`.`payment_method`,`bk_order`.`payment_gateway_status`,`bk_order`.`payment_gateway_id` FROM bk_order WHERE `reference`='"+orderId+"'";
    var SQL = db.query(sql_userId,function(error, results){
        ...
        let get_products = "SELECT products.name,products.price,products.discount,cart_product.cart_id,cart_product.product_id,cart_product.on_rental,sum(cart_product.quantity) as cartquantity, `cart`.cart_id as CartId,`cart`.user_id,`cart`.status FROM products LEFT JOIN cart_product ON products.id = cart_product.product_id LEFT JOIN cart ON cart_product.cart_id = cart.cart_id where cart.user_id='"+userId+"' and cart.`cart_id`='"+cart_id+"' group by product_id";
        var QUERY = db.query(get_products, function(error, get_orders){
            if(error) throw error;
            var SQL ="SELECT shipping_information.*,state.name as ShippingState FROM shipping_information LEFT JOIN state ON shipping_information.state = state.id WHERE shipping_information.`user_id`='"+userId+"'";
            
            var query = db.query(SQL, function(error,get_shipping_address){
                if(error) throw error;
                var SQL ="SELECT billing_information.*,state.name as BillingState FROM billing_information LEFT JOIN state ON billing_information.state = state.id WHERE billing_information.`user_id`='"+userId+"'";
                // console.log('SQL -->', SQL)
                var query = db.query(SQL, function(error,get_billing_address){
                    ...
                    var query = db.query(SQL, function(error, getEmail){
                        if(error) throw error;
                        res.render('admin/vieworder',{getEmail:getEmail[0],orderId:orderId,created_at:created_at,paid_amount:paid_amount,moment:moment,get_orders:get_orders,get_shipping_address:get_shipping_address[0],get_billing_address:get_billing_address[0],result:result});
                    });
                });
            });
        });
...
exports.editMembershipPlan = function(req, res){
	let sql = "UPDATE plans SET plan_name='"+req.body.plan_name+"', plan_price='"+req.body.plan_price+"', total_month='"+req.body.plan_months+"', total_book='"+req.body.total_book+"', monthly_book='"+req.body.monthly_book+"', is_deleted='"+req.body.is_active+"' WHERE id="+req.body.edit_id;
    ...
    if(req.url == '/addslider'){
        ...
        form.parse(req, function (err, fields, files) {
            ...
            fs.readFile(oldpath, function (err, data) {
                ...
                let sql1 = "INSERT INTO home_slider(`slider_name`,`link`,`descriptions`,`images`,`created_at`,`updated_at`) VALUES ('"+slider_name+"','"+link+"','"+descriptions+"','"+productimage+"','"+today+"','"+today+"')";
    ...
    if(req.url == "/updateslider"){
        ...
        form.parse(req, function (err, fields, files) {
            ...
            if(slider_img){
                ...
                let sql1 = "UPDATE `home_slider` SET `slider_name` ='"+slider_name+"', `descriptions` = '"+descriptions+"', `link` ='"+link+"', `images`= '"+slider_img+"' ,`status`='"+status+"', `updated_at`='"+today+"' WHERE id='"+edit_id+"'";
                ...
                let sql1 = "UPDATE `home_slider` SET `slider_name` ='"+slider_name+"', `descriptions` = '"+descriptions+"', `link` ='"+link+"', `status`='"+status+"', `updated_at`='"+today+"' WHERE id='"+edit_id+"'";
                    var query = db.query(sql1, function(error, update) {
                        if(error) throw error;
                        req.flash('message','Slider has been successfully updated.');
                        res.redirect("/homeslider");
...
exports.primeMembership = (req, res) => {
    if(req.method == "GET"){
        var sql = "SELECT `user_plan`.order_id as orderId,`user_plan`.user_id,`user_plan`.paid_amount,`user_plan`.payment_method,`user_plan`.created_at as orderDate, `users`.id as userId,`users`.name as username,`users`.email as useremail, `users`.mobile FROM `user_plan` LEFT JOIN `users` ON `user_plan`.user_id = `users`.id where `user_plan`.payment_method != 0 order by `user_plan`.order_id DESC ";
                    ...
                    var query = db.query(SQL, function(error, getEmail){
                        if(error) throw new Error('User Table Problem');
                            ...
                            var query = db.query(sql, function(error,getplanData){
                                ...
                                res.render('admin/viewprimeorder',{get_user_plan_data:get_user_plan_data[0],get_shipping_address:get_shipping_address[0],get_billing_address:get_billing_address[0],moment:moment,getEmail:getEmail[0],getplanData:getplanData[0],book_requestfromuser:book_requestfromuser,message:req.flash('message')});
                            });
                        });
                    });
                });
...
exports.addPrimeBook = (req,res, next) => {
    ...
    if(req.url == '/addprimebook'){
        ...
        var today = new Date().toISOString().slice(0, 19).replace('T', ' ');
            ...
            var sql = "INSERT INTO addprimebook(order_id,user_id,book_name,author,publisher,book_edition,isbn,message,created_at)VALUES('"+orderId+"','"+userId+"','"+book_name+"','"+author+"','"+publisher+"','"+book_edition+"','"+isbn+"','"+message+"','"+today+"')";
    ...
    if(req.url == "/delay_box_massege"){
        ...
        if(data.edit_message==""){
            ...
            query = db.query(sql, function(error,data){
                if(error) throw new Error('Insert problem in table delay_box');
                if(data){
                    var update = "UPDATE `bk_order` set `order_status`='8', `updated_at`='"+today+"' where reference='"+xss(req.body.edit_reference)+"' ";
    ...
    if(req.url == "/cancel_product_from_admin"){
        ...
        if(data.edit_c_message==""){
            ...
            query = db.query(sql, function(error,data){
                if(error) throw new Error('Insert problem in table cancel_product_from_admin');
                if(data){
                    var update = "UPDATE `bk_order` set `order_status`='9', `updated_at`='"+today+"' where reference='"+xss(req.body.edit_c_reference)+"' ";
...
exports.updateProductStatus = (req,res) => {
    ...
    if(req.url == "/update_product_status"){
        ...
        if(data.products_status==""){
            ...
            var sql = "UPDATE `bk_order` set `order_status`='"+xss(data.products_status)+"', `updated_at`='"+today+"' where reference='"+xss(data.edit_reference_de)+"' ";
