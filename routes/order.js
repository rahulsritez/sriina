...
exports.orderViewPage = (req, res, next) => {
  ...
  var SQL = db.query(sql_userId, function (error, results) {
    ...
    //       cart_product.on_rental,
    //       SUM(cart_product.quantity) AS cartquantity,
    ...
    //       products
    //   LEFT JOIN
    //       cart_product ON products.id = cart_product.product_id
    //   LEFT JOIN
    //       cart ON cart_product.cart_id = cart.cart_id
    //   WHERE
    //       cart.user_id = '${userId}'
...
exports.adminPlanPage = function (req, res) {
  ...
  let query = db.query(sql, function (error, result) {
    if (error) throw error;
    res.render("admin/adminplanpage", {
      title: title,
      getplans: result,
      message: req.flash("message"),
    });
...
exports.primeMembership = (req, res) => {
  ...
  if (req.method == "GET") {
    ...
    var query = db.query(sql, function (error, orderlist) {
      if (error) throw error;
      res.render("admin/primemembership", {
        orderlist: orderlist,
        moment: moment,
      });
    });
    ...
    var query = db.query(sql, function (error, get_user_plan_data) {
      ...
      var query = db.query(SQL, function (error, get_shipping_address) {
        ...
        var query = db.query(SQL, function (error, get_billing_address) {
          ...
          var query = db.query(SQL, function (error, getEmail) {
            console.log(
              "🚀 ~ file: order.js:225 ~ query ~ getEmail:",
              getEmail
            );
...
exports.addPrimeBook = (req, res, next) => {
  ...
  if (req.url == "/addprimebook") {
    ...
    var query = db.query(sql, function (error, insertdata) {
      ...
      req.flash(
        "message",
        "Books has been successfully added to Prime Id." + orderId
      );
  ...
  if (req.url == "/delay_box_massege") {
    ...
    if (data.edit_message == "") {
      ...
      query = db.query(sql, function (error, data) {
        ...
        if (data) {
          var update =
            "UPDATE `bk_order` set `order_status`='8', `updated_at`='" +
            today +
            "' where reference='" +
            xss(req.body.edit_reference) +
            "' ";
...
exports.cancelProductFromAdmin = (req, res) => {
  ...
  if (req.url == "/cancel_product_from_admin") {
    ...
    if (data.edit_c_message == "") {
      ...
      query = db.query(sql, function (error, data) {
        if (error)
          throw new Error("Insert problem in table cancel_product_from_admin");
        if (data) {
          var update =
            "UPDATE `bk_order` set `order_status`='9', `updated_at`='" +
            today +
            "' where reference='" +
            xss(req.body.edit_c_reference) +
            "' ";
    ...
    if (data.products_status == "") {
      ...
      query = db.query(sql, function (error, data) {
        ...
        if (data) {
          req.flash(
            "message",
            "Products status has been successfully updated."
          );
          res.redirect("/orderpage");
        }
      });
