...
exports.userSignIn = function(req, res, next){
    ...
    res.render('users/signin.ejs',{title:'signin form',success:req.flash('success'),errors:req.flash('errors'), message:req.flash('message'), csrfToken: req.csrfToken(),formregister_errors:''});
};
    ...
    if(req.method == "POST"){
        ...
        if(!useremail || useremail.search(/\w/)< 0){
            ...
            res.redirect('/sign-in');
                ...
                var sql ="select email,mobile from users where email='"+useremail+"' or mobile='"+userphone+"'";
                var query = db.query(sql, function(error, result) {
                    if(error) throw new Error('User Data Problem');
                    if(result.length>0){
                        req.flash('errors','User already registered with same email id or mobile');
                        res.redirect('/sign-in');
                    } else {
                    let register_sms = false;
                        var sql1 = "INSERT INTO `users`(`name`,`email`,`mobile`,`type`, `password`,createdby,modifiedby,ip_address) VALUES ('"+username+"', '"+useremail+"', '"+userphone+"','0', '"+hash+"','"+today+"','"+today+"','"+clientIp+"')";
                        var query = db.query(sql1, async function(err, result) {
                            if(error) throw new Error('Data cannot inserted.');

			     const bodyContent = "Sriina: Dear, "+username+" Thank you for registering with us your account has been successfully created username: "+useremail+" and password: "+pass+""
				...
				auth: {
				   ...
				   pass: process.env.EMAIL_PASSWORD
                              ...
                              var mailOptions = {
                                ...
                                html:compiledTemplate.render({username:username,useremail:useremail,pass:pass}),
                              };

                              transporter.sendMail(mailOptions, function(error, info){
                                  if (error) {
                                    console.log('mail error send---->', info);
                                    req.flash("errors","We are sorry we're not able to identify you given the information provided.");
                                    res.redirect('/sign-in');
                                  } else {
                                      console.log('Email sent: ' + info.response);
                                      console.log('sent', info);
                                      req.flash('message','Your registration has been completed successfully.');
                                      if(register_sms) req.flash('message','Your registration has been completed successfully');
                                      res.redirect('/sign-in');
                                  }
                              });
...
exports.deleteInCart = (req, res, next) => {
    if(req.method == "POST"){
        ...
        if(userId == null){
            ...
            var sql = "DELETE `cart_product`  FROM `cart_product`  LEFT JOIN `cart` ON `cart_product`.cart_id = `cart`.cart_id WHERE `cart`.user_id ='"+userId+"' and `cart_product`.product_id = '"+product_id+"'";
...
exports.addCallBack = (req, res, next) => {
    if(req.method == "POST"){
        ...
        var today = new Date().toISOString().slice(0, 19).replace('T', ' ');
        var sql1 = "INSERT INTO `request_call_back`(`product_id`,`username`,`userphone`,`created_at`,`updated_at`) VALUES ('"+product_id+"', '"+username+"', '"+userphone+"','"+today+"','"+today+"')";
                var query = db.query(sql1, function(err, result) {
                    if(err){
                        throw err;
                    } else {
                        req.flash('message','Your request has been sent to admin they will call back soon.');
                        ...
                        var query = db.query(get_count_total, function(error, totalAmount){
                            if(error){
                                ...
                                totalAmount.forEach((list, index)=>{
                                    ...
                                    let newTotalGrocey = (list.groceyProductPrice) * (list.cartquantity);
                                    let afterdiscountGrocery = (newTotalGrocey)-(newTotalGrocey/100)*list.unitDiscount;
                                ...
                                var query = db.query(SQL_RENT_STATE, function(error, get_rent_state){
                                    ...
                                    var SQL ="SELECT `shipping_information`.*,`state`.name as ShippingState, `country`.name as shippingCountyName FROM `shipping_information` LEFT JOIN `state` ON `shipping_information`.state = `state`.id LEFT JOIN `country` ON `state`.countryid = `country`.id WHERE shipping_information.`user_id`='"+userId+"' ";

                                    var query = db.query(SQL, function(error, getshipping){
                                        if(error) throw error;
                                        var SQL ="SELECT `billing_information`.*,`state`.name as ShippingState, `country`.name as billingCountyName FROM `billing_information` LEFT JOIN `state` ON `billing_information`.state = `state`.id LEFT JOIN `country` ON `state`.countryid = `country`.id WHERE billing_information.`user_id`='"+userId+"' ";

                                        var query = db.query(SQL, function(error, getbilling){
                                            if(error) throw error;
                                            let permission = "SELECT * FROM settings where id=1";
                                            var query = db.query(permission, function(error, getpermission){
                                                if(error) throw new Error('Permission Page Error');
                                                res.render('shoppingcart/checkout',{cryptr:cryptr,'CartId':CartId,'txnid':txnid,'total_amount':total_amount,'result':result[0],delivery_charge:delivery_charge,subtotal:subtotal,'message':req.flash('message'), 'errors':req.flash('errors'),'get_state':getstate,'total':parseFloat(total),'captchaImage':svgTag,'errorStatus':'',on_rental:on_rental,get_rent_state:get_rent_state,getshipping:getshipping,getbilling:getbilling,csrfToken: req.csrfToken(),getpermission:getpermission[0]});
                                            });
                                        });
                                    });
...
exports.billingInformation = (req, res, next) => {
    ...
    if(req.method == "POST"){
        ...
        if(userId==null){
            ...
            var sql = "INSERT INTO `billing_information`(`user_id`,`fullname`,`mobile`,`pincode`,`house_address`, `colony_area`,landmark,city_town,state,created_at,updated_at) VALUES ('"+userId+"', '"+fullname+"', '"+mobile+"','"+pincode+"', '"+house_address+"', '"+colony_area+"', '"+landmark+"', '"+city_town+"', '"+state+"','"+today+"','"+today+"')";
            var query = db.query(sql, function(error, result){
                if(error){
                    throw error;
                } else {
                    if(membershipcheckout_billing==1){
                        req.flash('message','Your Billing Information has been succefully added.');
...
exports.shippingInformation = (req, res, next) => {
    ...
    if(req.method == "POST"){
        ...
        if(userId==null){
            ...
            var sql = "INSERT INTO `shipping_information`(`user_id`,`fullname`,`mobile`,`pincode`,`house_address`, `colony_area`,landmark,city_town,state,created_at,updated_at) VALUES ('"+userId+"', '"+fullname+"', '"+mobile+"','"+pincode+"', '"+house_address+"', '"+colony_area+"', '"+landmark+"', '"+city_town+"', '"+state+"','"+today+"','"+today+"')";
            var query = db.query(sql, function(error, result){
                if(error){
                    throw error;
                } else {
                    if(membershipcheckout_shipping == 1){
                        req.flash('message','Your Shipping Information has been succefully added.');
                        res.redirect('membership_checkout');
                    } else {
                        req.flash('message','Your Shipping Information has been succefully added.');
                        res.redirect('checkout');
    ...
    if(userId == null){
        ...
        var query = db.query(sql1, async function(error, result) {
            ...
            var product_details = db.query(get_products, async function(err, product) {
                ...
                const cartProduct = product.map(data => {
                    ...
                    let prepareObject ={
                        price_data: {
                            ...
                            unit_amount: parseInt(data.price * ((100-data.discount) / 100)) * 100,
                        },
                ...
                var sql = "UPDATE bk_order SET `payment_gateway_id` ='"+session.id+"' WHERE `order_id` ='"+orderId+"'";
...
exports.orderPaymentStatus = (req, res, next) => {
    ...
    if(status) {
        var update_cart = "UPDATE `cart` SET status='1' WHERE cart_id = '"+cartId+"'";
    ...
    if(userId == null){
        ...
        if(txtid==""){
            ...
            res.redirect('checkout');
                ...
                var sql1 = "INSERT INTO `bk_order`(`cart_id`,`reference`,`paid_amount`,`customer_id`,`payment_method`,created_at,updated_at) VALUES ('"+cartId+"','"+txtid+"', '"+paid_amount+"', '"+userId+"','"+payment_method+"','"+today+"','"+today+"')";
                //console.log(sql1); return;
                var query = db.query(sql1, function(error, result) {
                    if(error) throw error;

                        var sql = "UPDATE `cart` SET status=1 WHERE cart_id='"+cartId+"' and user_id='"+userId+"'";
                        var query = db.query(sql, function(error, updatecart){
                            if(error) throw error;
                            var user_plan = "UPDATE user_plan SET status=1, payment_method='2' WHERE order_id='"+txtid+"'";
                            var query = db.query(user_plan, function(error, updatepnal) {
                                if(error){
                                    console.log('Something went wrong please try again.');
                                } else {
                                    var sqlQuery = "select * from bk_order WHERE `order_id` ='"+result.insertId+"'";
                                    let orderSMS = false;
                                    db.query(sqlQuery, async function(error, orderData){
                                        if(error) throw error
						...
						let mail_data = JSON.parse(JSON.stringify(get_orders))
						ejs.renderFile("./views/emails/order_confirmed.ejs", { username: userquerys[0].name,get_orders:mail_data}, function (err, data) {
		                                    if (err) {
...
exports.PrimeConfimOrder = (req, res, next) => {
    ...
    if(req.method == "POST"){
        ...
        if (req.body.captchaText != captchaText) {
            ...
            var userData = "SELECT `plans`.total_month,`user_plan`.created_at as planDate,`user_plan`.status FROM `user_plan` LEFT JOIN `plans` ON `user_plan`.plan_id = `plans`.id WHERE `user_plan`.order_id='"+txnid+"' and `user_plan`.status='0' ";
            var query = db.query(userData, function(error, results){
                ...
                if(results){
                    var sql1 = "UPDATE `user_plan` SET `paid_amount`='"+pay_amount+"', `payment_method`='2', status='1', expired_date = '"+futureMonth+"' WHERE order_id = '"+txnid+"' and user_id='"+userId+"' ";
                    var mysql = db.query(sql1,  function(error, getupdateplan){
                        if(error) throw error;
                        req.flash("message","Sriina online prim membership has been successfully done your application.");
            ...
            const plan = db.query(userData, async function(err, planData) {
                ...
                instance.paymentLink.create({
                    ...options
                        ...
                        let futureMonth = moment(plan_buy_date).add(expired_date, 'M').format('YYYY-MM-DD h:mm:ss');
                        if(planData){
                            var sql1 = "UPDATE `user_plan` SET `paid_amount`='"+pay_amount+"', `payment_method`='1', `payment_gateway_id` ='"+dataMain.id+"', expired_date = '"+futureMonth+"' WHERE order_id = '"+txnid+"' and user_id='"+userId+"' ";
...
exports.membershipPaymentStatus = (req, res, next) => {
    ...
    if(status) {
        var update_cart = "UPDATE `user_plan` SET status='1' WHERE order_id = '"+orderId+"'";
        var query = db.query(update_cart, function(error, results){
            if(error) throw error;
        })
    }
    ...
    var sqlQuery = "select `plans`.`plan_name`, `plans`.`plan_price`, `plans`.`total_month`, `user_plan`.`order_id`, `user_plan`.`expired_date` from user_plan LEFT JOIN `plans` ON `user_plan`.plan_id = `plans`.id WHERE `user_plan`.order_id='"+orderId+"'";
    let orderSMS = false;
    db.query(sqlQuery, async function(error, orderData){
        if(error) throw error

        const url = `${URL}/sritezprime`
        const bodyContent = "Hello "+req.session.user.name+" \n Your order id : "+orderData[0].order_id+" \n View order: "+url+" \n Membership plan: "+orderData[0].plan_name+" \n Thanks for membership!"
        if(req.session.user.mobile) {
            await client.messages
                .create({
                    body: bodyContent,
        ...
        var sql = "UPDATE user_plan SET `payment_gateway_status` ='"+payment_gateway_status+"' WHERE `order_id` ='"+orderId+"'";
        var query = db.query(sql, function(error, updatecart){
            if(error) throw error
            if(!orderSMS) req.flash('errors','Your register mobile number wrong! So, not able to send order conformation SMS');
            if(orderSMS) req.flash('message','Membership conformation SMS send successfully');
            req.flash('message','Sriina online prim membership has been successfully done your application.');
            res.redirect('/sritezprime');
        })
    })
...
exports.forgotPassword = (req,res,next) => {
    if(req.method == "GET"){
        ...
        let query = db.query(sql2, function(error, getcategory){
            res.render('front/forgot_password.ejs',{'categorylist': getcategory, title:'Forgot Password',csrfToken: req.csrfToken(),'message':req.flash('message')});
        });
    }
}
                ...
                transporter.sendMail(mailOptions, function(error, info){
                    if (error) {
                        ...
                        var query = db.query(sql, function(error, updateusers){
                            ...
                            req.flash('message','OTP sent to your registered email id and also check all folder. ');
...
exports.otpVerify = (req, res, next) => {
    if(req.method == 'GET'){
        let sql2 = "SELECT * from books_category where is_deleted=0";
        let query = db.query(sql2, function(error, getcategory){
            res.render('front/verifyotp',{'categorylist': getcategory, title:'Verify OTP',csrfToken: req.csrfToken(),'message':req.flash('message')});
        });
    }
}
...
exports.verifiedOTP = (req, res, next) => {
    if(req.method == "POST"){
        ...
        var sql ="SELECT otp FROM `users` WHERE `email`='"+req.session.sessionEmail+"' and `type`= '"+0+"'";
        //console.log(sql);
...
exports.resetPWD = (req,res,next) => {
    if(req.method == 'GET'){
        let sql2 = "SELECT * from books_category where is_deleted=0";
        let query = db.query(sql2, function(error, getcategory){
            res.render('front/resetpwd',{'categorylist': getcategory, title:'Reset Password',csrfToken: req.csrfToken(),'message':req.flash('message')});
        });
    }
}
...
exports.resetPwdSave = (req, res, next) => {
    if(req.method == "POST"){
        ...
        //console.log(otp); return;
        var sql ="SELECT email FROM `users` WHERE `email`='"+req.session.sessionEmail+"' and `type`='"+0+"'";
        ...
        var query = db.query(sql, function(error, getrows){
            ...
            if(getrows.length>0){
                if(password != confirm_password){
                    ...
                    req.flash('message',"You have entered less than 5 characters for password");
                    res.redirect('resetpwd');
                } else {
                    var sqlpwd = "UPDATE `users` SET password='"+hash+"' WHERE `email`='"+getrows[0].email+"'"
                    var mysql = db.query(sqlpwd, function(error, updatepwd){
                        if(error) {
                            throw error;
                        } else {
                            req.flash('message',"Your password has been changed successfully! Thank you");
                            res.redirect('sign-in');
                        }
                    });
                }
...
exports.registerUserPlan = (req, res, next) => {
    if(req.method == 'POST'){
        ...
        var sqlplan = "SELECT `plan_id` from `user_plan` WHERE `user_id` = '"+userId+"' AND `status`='1' ";
        var query = db.query(sqlplan, function(error, get_plans){
            if(userId == null){
                ...
                res.redirect('sritezprime');
                    ...
                    orderId = 'STZ'+ orderId;
                    var insert_plan = "INSERT INTO `user_plan`(order_id,plan_id,user_id,status,created_at) VALUES('"+orderId+"','"+plan_id+"','"+userId+"','0','"+today+"')";
        ...
        var query = db.query(SQL_RENT_STATE, function(error, prime_rent_state){
            ...
            var query = db.query(SQL, function(error, getshipping){
                ...
                var query = db.query(SQL, function(error, getbilling){
                    ...
                    var userData = "SELECT `plans`.plan_price,`user_plan`.plan_id,`user_plan`.order_id FROM `user_plan` LEFT JOIN `plans` ON `user_plan`.plan_id = `plans`.id WHERE `user_plan`.user_id='"+userId+"' and `user_plan`.status='0' ";
                    ...
                    var query = db.query(userData, function(error, getPlanData){
                        ...
                        var query = db.query(sql, function(error,results){
                            if(error) throw error;
                            res.render('shoppingcart/membership_checkout',{'txnid':txnid,'total':planPrice,result:results[0],'message':req.flash('message'),'captchaImage':svgTag,prime_rent_state:prime_rent_state,getshipping:getshipping,getbilling:getbilling,csrfToken: req.csrfToken(),getcartdata:''});
                        });
                    });
                });
            });
...
exports.userContactUs = (req, res, next) => {
    ...
    res.render('front/contactus.ejs', {title:title,csrfToken: req.csrfToken(),'message':req.flash('message'),getcartdata:''});
}
...
exports.userContactUsSave = (req, res,next) => {
    if(req.method == 'POST'){
        ...
        var contactus = "INSERT INTO `contactus`(name,email,phone,subject,message,created_at) VALUES('"+name+"','"+email+"','"+mobile+"','"+subject+"','"+message+"','"+today+"')";
            var query = db.query(contactus, function(error,plan_results){
                if(error) throw error;
                req.flash('message',"We appreciate you contacting us [ "+name+" ] One of our colleagues will get back in touch with you soon!");
...
exports.deleteShippingAddress = (req, res, next)=>{
    ...
    if(userId==null){
        ...
        var sql = "DELETE FROM `shipping_information` WHERE `id` = '"+shippingId+"' and user_id='"+userId+"'";
...
exports.deleteBillingAddress = (req, res, next)=>{
    ...
    if(userId==null){
        ...
        var sql = "DELETE FROM `billing_information` WHERE `id` = '"+billingId+"' and user_id='"+userId+"'";
        var query = db.query(sql, function(error, deletequery){
            if(error) throw error;
            res.end('{"status": 1}');
        });
...
exports.vendorRegister = (req,res,next)=>{
    var svgTag = getCaptcha().data;
    res.render('users/vendor_register',{title:'Vendor Registration form',message:req.flash('message'), csrfToken: req.csrfToken(),'captchaImage':svgTag});
}
...
exports.vendorRegisterFrm = (req,res,next)=>{
    if(req.method == 'POST'){
        ...
        let company_contact_person  = xss(data.company_contact_person.trim().toLowerCase());
        let company_contact_phone_no = xss(data.company_contact_phone_no.trim().toLowerCase());
        ...
        if(!company_mail || company_mail.search(/\w/)< 0){
            ...
            res.redirect('/vendor_register');
        } else if(!company_contact_person || company_contact_person.search(/\w/)< 0){
            req.flash('message','Company contact person cannot be empty.');
            res.redirect('/vendor_register');
        } else if(!company_contact_phone_no || company_contact_phone_no.search(/\w/)< 0){
            ...
            var sql ="SELECT `email`,`mobile` FROM `users` where `email`='"+company_mail+"' or `mobile`='"+company_phone+"'";
            var query = db.query(sql, function(error, result) {
                if(error) throw new Error('User Data Problem');
                if(result.length>0){
                    ...
                    let usertbl = "INSERT INTO `users`(name,email,mobile,password,type,status,createdby) VALUES('"+company_name+"','"+company_mail+"','"+company_phone+"','"+hash+"','3','1','"+today+"')";
                    db.query(usertbl,function(error,datainserted){

                        let vendor = "INSERT INTO `vendor_info`(user_id,company_mail,company_name,company_phone,company_contact_person,company_contact_phone_no,company_gst,vender_product_type,created_at) VALUES('"+datainserted.insertId+"','"+company_mail+"','"+company_name+"','"+company_phone+"','"+company_contact_person+"','"+company_contact_phone_no+"', '"+company_gst+"', '"+product_type+"', '"+today+"')";
                        let vendor_query = db.query(vendor,function(error,insertVendor){
                            ...
                            var transporter = nodemailer.createTransport({
                              ...
                              let msg = 'Dear '+company_contact_person+',\nWe appreciate you contacting us '+company_name+'\One of our colleagues will get back in touch with you soon. \n \n \n Have great day \n Sriina Online Team';
                            ...
                            req.flash('message','Your request has been sent to admin they will contact you soon.');
                            res.redirect('/vendor_register');
                        });
                    });
                }
